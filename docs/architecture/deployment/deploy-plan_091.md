...
1. **Interceptação Inteligente de URLs de Cache Vazio**:
   - Criada a action [`ImageCacheAction`](file:///var/www/html/agsonhos/backend/core/Controller/Actions/Common/ImageCacheAction.php) mapeada nas rotas `/image/cache/{path:.+}` e `/{lang}/image/cache/{path:.+}` em [`Routes.php`](file:///var/www/html/agsonhos/backend/Config/Routes.php).
...
2. **Geração Instantânea e Escrita em Disco**:
   - O [`ImagePresenter`](file:///var/www/html/agsonhos/backend/core/Support/Presenters/ImagePresenter.php) analisa o caminho, localiza a imagem de origem (ou fallback `placeholder.png` / `no-image.png`), gera a miniatura redimensionada com GD e salva o arquivo fisicamente na pasta `public_html/image/cache/...`.
...
### 🧪 Testes e Validação
- Criada a suíte de testes [ImageCacheOnDemandTest.php](file:///var/www/html/agsonhos/tests/Validation/ImageCacheOnDemandTest.php).
...
- **Causa Raiz**: O helper [`Customer`](file:///var/www/html/agsonhos/backend/core/Support/Customer.php) consultava apenas `$_SESSION` e não o Redis, fazendo com que `$customer->isLogged()` retornasse `false` nas subpáginas.
...
- **Solução**: O script [`public_html/clean_cache.php`](file:///var/www/html/agsonhos/public_html/clean_cache.php) e [`backend/clean_twig_cache.php`](file:///var/www/html/agsonhos/backend/clean_twig_cache.php) agora limpam todas as camadas de cache (Twig `twig_slim` e `twig_setup`, arquivos `*.cache` e `*.json` do Core, proxies em `alpha_proxies`, miniaturas em `image/cache`, OPcache do PHP e chaves de cache do Redis preservando sessões).
...
1. **Nova Action Sob Demanda ([`ImageCacheAction.php`](file:///var/www/html/agsonhos/backend/core/Controller/Actions/Common/ImageCacheAction.php))**:
...
2. **Aprimoramento do [`ImagePresenter.php`](file:///var/www/html/agsonhos/backend/core/Support/Presenters/ImagePresenter.php)**:
