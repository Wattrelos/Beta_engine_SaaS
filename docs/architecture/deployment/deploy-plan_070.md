...
Este plano detalha a criação de **6 novos diagramas de atividades em PlantUML (.puml)** no diretório [docs/workflows/activity_diagrams/](/docs/workflows/activity_diagrams/), utilizando a sintaxe moderna de diagramas de atividade do PlantUML (`start`, `if/then/else`, `fork/again`, `repeat/while`, `stop`).
