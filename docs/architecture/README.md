...
*   **Ponto de Entrada (Bootstrap):** [`public_html/index.php`](/public_html/index.php) (Front-end) e [`public_html/LPDHED2dC7Gjrg2b/index.php`](/public_html/LPDHED2dC7Gjrg2b/index.php) (Admin Ofuscado)
*   **Especificações de Contrato (Spec-Driven):** [`docs/specs/`](/docs/specs/) (OpenAPI 3.1, JSON Schema DTOs, Gherkin BDD)
*   **Configuração de Rotas:** [`Config/Routes.php`](/Config/Routes.php)
*   **Camada de Controladores (Actions):**
    *   **Frontend (E-commerce):** [`core/Controller/Actions/`](/core/Controller/Actions/) (Namespace `Alpha\Controller\Actions\`)
    *   **Painel Admin:** [`core/Admin/Controllers/Actions/`](/core/Admin/Controllers/Actions/) (Namespace `Alpha\Admin\Controllers\Actions\`)
        *   [`User/User/`](/core/Admin/Controllers/Actions/User/User/) - Gestão de Funcionários
        *   [`User/UserGroup/`](/core/Admin/Controllers/Actions/User/UserGroup/) - Gestão de Papéis & Permissões
*   **Apresentação (Views Twig):** [`resources/views/`](/resources/views/)
*   **Camada de Domínio (DDD):**
    *   **Repositories:** [`core/Model/Domain/Repositories/`](/core/Model/Domain/Repositories/) (`UserRepository.php`, `UserGroupRepository.php`, `CartRepository.php`, etc.)
    *   **Domain Entities (POPOs PHP 8.4):** [`core/Model/Domain/Entities/`](/core/Model/Domain/Entities/) (`User.php`, `UserGroup.php`)
        *   [`Customer/`](/core/Model/Domain/Entities/Customer/) - Entidades de Clientes
        *   [`Supplier/`](/core/Model/Domain/Entities/Supplier/) - Entidades de Fornecedores
        *   [`Geo/`](/core/Model/Domain/Entities/Geo/) - Entidades Geográficas
*   **Camada de Persistência:**
    *   **Data Mappers:** [`core/Mappers/EntityMappers/`](/core/Mappers/EntityMappers/) (`UserMapper.php`, `UserGroupMapper.php`)
    *   **DataAccessObject (DAO):** [`core/Model/DataAccessObject/`](/core/Model/DataAccessObject/)
*   **Gerenciador de Container (Legacy & DI):** [`core/AlphaContainer.php`](/core/AlphaContainer.php) e [`Containers/AppContainer.php`](/Containers/AppContainer.php)
...
```
*   **Classe Base:** As ações estendem [`BaseController.php`](/core/Controller/BaseController.php) e implementam [`ActionInterface.php`](/core/Controller/Actions/ActionInterface.php).
...
*   **Repository Pattern:** Abstrai e centraliza a busca e persistência de agregados de negócio. Exemplo: [`CartRepository.php`](/core/Model/Domain/Repositories/CartRepository.php).
*   **Data Mapper:** Desacopla as Entidades de Domínio da estrutura do banco. As queries SQL residem inteiramente nos mappers em [`core/Mappers/EntityMappers/`](/core/Mappers/EntityMappers/).
*   **Identity Map:** Cache em memória RAM no ciclo de vida do request implementado no [`DataAccessObject.php`](/core/Model/DataAccessObject/DataAccessObject.php) para evitar queries N+1 duplicadas.
*   **Proxy Pattern & Lazy Loading:** Relacionamentos complexos (`#[ManyToOne]`, `#[OneToMany]`) utilizam classes Proxy geradas pela [`ProxyFactory.php`](/core/Model/DataAccessObject/ProxyFactory.php).
*   **Unit of Work (UoW):** Encapsula o controle de transações (PDO `beginTransaction()`, `commit()`, `rollBack()`) na [`UnitOfWork.php`](/core/Model/DataAccessObject/UnitOfWork.php) para atomicidade multi-tabelas.
...
*   **Pseudo-Null (FK = 0):** O banco legado usava `0` para ausência de chave estrangeira. O [`DataAccessObject.php`](/core/Model/DataAccessObject/DataAccessObject.php) intercepta a hidratação recursiva e converte `0` para `null`. Adicionalmente, colunas críticas (`parent_id` na categoria, `manufacturer_id` no produto) foram migradas para permitir `NULL` nativo com restrições `FOREIGN KEY` reais.
*   **Session Failsafe (Prevenção de Estouro):** O [`SessionMapper.php`](/core/Mappers/EntityMappers/SessionMapper.php) inspeciona o tamanho da sessão via `SELECT LENGTH(data)`. Se for maior que `5MB`, a linha é apagada e a sessão recriada instantaneamente para prevenir crash de RAM.
...
Todas as rotas estão centralizadas em [`Config/Routes.php`](/Config/Routes.php).
